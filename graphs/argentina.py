...
# mean norm function for columns
...
argentina_df = normalized_df[normalized_df['Country Name'] == 'ARGENTINA']
...
def plot_radar_chart(data, metricsfun, title):
    ...
    color_palette = ['#6cace4', '#0500FF', '#9CDADB', '#FF00DE', '#FF9900', '#FFFFFF']
    ...
    plt.yticks([0, 0.25, 0.5, 0.75, 1], ["","","","",""], color="white", size=12, fontproperties = prop)
    plt.xticks(theta[:-1], metricsfun, color='white', size=12, fontproperties = prop)
    plt.legend(loc='upper right', bbox_to_anchor=(1.3, 1.1)) 
    plt.gca().set_facecolor('black')
    plt.title("Argentina Tournament Performance", color='white', fontsize = 30, pad=45, fontproperties = prop)
    return fig

fig = plot_radar_chart(argentina_df, metrics_list, title="Soccer Radar Chart")
plot_path = os.path.join("images", "argentina_spider.png")
