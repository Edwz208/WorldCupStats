...
for i in range(len(x)):
   statistic[df.loc[i, "team1"]]+=df.loc[i, "crosses completed team1"]
   statistic[df.loc[i, "team2"]]+=df.loc[i, "crosses completed team2"]
...
plt.title("Tournament Crossing Performance", fontproperties=prop, color='white', fontsize = '22')
plt.ylabel("Crosses Completed", fontproperties = prop, fontsize = 22, color = 'white', labelpad=25)
plt.xlabel("Teams", fontproperties = prop, fontsize = 22, color = 'white')



plot_path = os.path.join("images", "crosses.png")
